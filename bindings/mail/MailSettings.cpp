...
class PyBMailAddOnSettings : public BMailAddOnSettings{
	public:
        using BMailAddOnSettings::BMailAddOnSettings;
        status_t			Load(const BMessage& message) override {
            PYBIND11_OVERLOAD(status_t, BMailAddOnSettings, Load, message);
        }
        status_t			Save(BMessage& message) override {
            PYBIND11_OVERLOAD(status_t, BMailAddOnSettings, Save, message);
        }
        bool				HasBeenModified() const override {
            PYBIND11_OVERLOAD(bool, BMailAddOnSettings, HasBeenModified);
        }
};

class PyBMailProtocolSettings : public BMailProtocolSettings{
	public:
        using BMailProtocolSettings::BMailProtocolSettings;
        status_t			Load(const BMessage& message) override {
            PYBIND11_OVERLOAD(status_t, BMailProtocolSettings, Load, message);
        }
        status_t			Save(BMessage& message) override {
            PYBIND11_OVERLOAD(status_t, BMailProtocolSettings, Save, message);
        }
        bool				HasBeenModified() const override {
            PYBIND11_OVERLOAD(bool, BMailProtocolSettings, HasBeenModified);
        }
};
...
py::class_<BMailAddOnSettings, PyBMailAddOnSettings, BMessage>(m, "BMailAddOnSettings")
.def(py::init(), "")
...
py::class_<BMailProtocolSettings, PyBMailProtocolSettings, BMailAddOnSettings>(m, "BMailProtocolSettings")
.def(py::init(), "")
