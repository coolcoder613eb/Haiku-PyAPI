...
This property returns the global variable ``be_clipboard`` which holds 
the ``BClipboard`` created for you by your ``BApplication`` object.

.. warning::
   Remember that you need a ``BApplication`` to obtain the ``be_clipboard``
   with this property.
   
.. note::
   This is an example how to use this property:
   
   .. code-block:: python
   
      # *** BApplication should already exist ***
      from Be import BClipboard
      global be_clipboard
      be_clipboard=BClipboard.be_clipboard
      # your example code ...
