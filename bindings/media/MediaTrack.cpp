...
py::class_<BMediaTrack, std::unique_ptr<BMediaTrack,py::nodelete>>(m, "BMediaTrack")
...
.def("ReadFrames", py::overload_cast<void *, int64*, media_header *>(&BMediaTrack::ReadFrames), "", py::arg("buffer"), py::arg("_frameCount"), py::arg("header")=NULL)
.def("ReadFrames", py::overload_cast<void *, int64*, media_header *, media_decode_info *>(&BMediaTrack::ReadFrames), "", py::arg("buffer"), py::arg("_frameCount"), py::arg("header"), py::arg("info"))
...
.def("ReadChunk", [](BMediaTrack &self) {
	char* buffer = nullptr;
	int32 size = 0;
	status_t status = self.ReadChunk(&buffer, &size, nullptr);
	if (status == B_OK) {
		return std::make_tuple(status, py::bytes(buffer, size));
	}
	else {
		return std::make_tuple(status, py::bytes());
	}
},"")
...
.def("GetParameterWeb",  [](BMediaTrack &self) {
